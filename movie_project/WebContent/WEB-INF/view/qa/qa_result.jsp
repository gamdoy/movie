...
function replyUrl(){
	...
	window.location="<%=request.getContextPath() %>/qa/login/reply.do?fqParentNumber="+fqParentNumber+"&fqNo="+fqNo;
}
...
function modifyUrl(){
	...
	var number = ${requestScope.qa_vo.fqNo }
	var fileName = ${requestScope.svo.fileName }
	window.location="<%=request.getContextPath() %>/qa/login/modify.do?number="+number+"&fileName="+fileName;
}
...
<c:choose>
	...
	<form method="post" action="<%=request.getContextPath()%>/qa/login/comment.do"  id="qa_commentForm">
